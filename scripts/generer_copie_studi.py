...
for label, value in [
    ("Dépôt GitHub public", "https://github.com/mehdi-prieux/vite-et-gourmand"),
...
doc.add_heading("Partie 2 Spécifications techniques", level=1)
...
add_question(doc, "2 Environnement de travail", """Le dépôt Git contient frontend/, backend/, database/mysql/, docs/, tests/ et output/pdf/. Le serveur local se lance depuis la racine avec php -S 127.0.0.1:8000. Les connexions utilisent des variables d'environnement et non des secrets versionnés ; .env.example documente les paramètres. Le README détaille l'installation et les scripts SQL de création et d'insertion. Les tests avec écriture vérifient que SELECT DATABASE() renvoie vite_gourmand_test avant toute mutation. La branche de travail historique est fix/database-config-env ; l'organisation Git réellement publiée doit être vérifiée sur le dépôt.""")
...
doc.add_heading("Partie 3 Recherche", level=1)
...
add_question(doc, "2 État de remise", """Les parcours locaux testés et les documents disponibles sont distingués des points non validés : synchronisation CouchDB réelle, transport e-mail réel, identité légale définitive et recette sur URL publique. Le dépôt GitHub doit contenir les commits de cette version avant remise. Aucun résultat de déploiement ou de statistique réelle n'est affirmé sans preuve.""")
