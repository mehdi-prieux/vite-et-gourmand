...
doc.add_paragraph(
    ...
    "23 septembre 2026. Les éléments non validés sont indiqués comme tels."
)
...
for label, value in [
    ...
    ("Application publique", "https://vite-et-gourmand.42web.io/frontend/ — accueil, menus, horaires et connexion administrateur vérifiés le 23 septembre 2026"),
    ("Administrateur de démonstration", "admin@test.com / Admin!Demo2026 — connexion vérifiée sur l'application publique, compte réservé à la démonstration"),
]:
    p = doc.add_paragraph()
...
L'application repose sur une interface HTML, CSS et JavaScript, une API PHP et une base MySQL pour les opérations métier. Une projection documentaire minimale vers CouchDB est préparée pour les statistiques ; sa validation de bout en bout sur une instance réelle reste à effectuer. Les mots de passe sont hachés, les accès contrôlés par rôle et les écritures authentifiées protégées contre les requêtes intersites. Les parcours principaux ont été testés localement sur une base dédiée. L'accueil, les menus, les horaires et la connexion administrateur ont aussi été vérifiés sur l'URL publique. Les e-mails ont été vérifiés en mode journal de test, pas encore avec un transport réel.""")
...
add_question(doc, "2 État de remise", """Les parcours locaux testés et les documents disponibles sont distingués des points non validés : synchronisation CouchDB réelle, transport e-mail réel, identité légale définitive et recette hébergée complète. L'application est accessible en HTTPS ; l'accueil, les menus, les horaires et la connexion administrateur ont été vérifiés le 23 septembre 2026. Les statistiques CouchDB y sont explicitement indisponibles. La branche publique livraison-studi contient les commits de cette version ; main reste inchangée et la PR n°2 n'est pas fusionnée.""")
