...
| Dépôt, outil de projet et application publics | [Branche publique de livraison](https://github.com/mehdi-prieux/vite-et-gourmand/tree/livraison-studi) ; [suivi public](https://github.com/mehdi-prieux/vite-et-gourmand/issues) | `private=false` et branche `livraison-studi` publiquement vérifiés ; PR n°2 encore ouverte/non fusionnée ; aucune URL d'application publique | Partiel | Choisir l'hébergement gratuit et obtenir l'URL publique ; domaine personnalisé facultatif |
...
- Commits thématiques publiés sur `livraison-studi` : (1) API, schéma SQL et recette ; (2) interfaces ; (3) documentation et livrables. L'intégration CouchDB reste explicitement partielle tant qu'une instance réelle n'a pas été testée. Aucun secret n'est volontairement ajouté ; la copie Word nominative est ignorée par Git.
- Branches : `main`, `develop` et les branches de fonctionnalité existent réellement ; `livraison-studi` porte la version à remettre. `main` n'a pas été modifiée et la PR n°2 reste ouverte et non fusionnée.
